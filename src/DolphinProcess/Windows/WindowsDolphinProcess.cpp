...
bool WindowsDolphinProcess::findPID(std::vector<std::string>& dolphin_names)
{
  ...
  // combine the default vector with the additional vector and convert it to a wstring vector
  std::vector<std::string> default_names = { "Dolphin.exe", "DolphinQt2.exe", "DolphinWx.exe" };
  std::vector <std::string> all_names;
  all_names.reserve(default_names.size() + dolphin_names.size());
  all_names.insert(all_names.end(), default_names.begin(), default_names.end());
  all_names.insert(all_names.end(), dolphin_names.begin(), dolphin_names.end());
  ...
  if (Process32First(snapshot, &entry) == TRUE)
  {
    ...
    {
      bool found = false;
      for (std::string w : all_names) {
          found = found || std::wstring(entry.szExeFile) == std::filesystem::path(w).wstring();;
      }
